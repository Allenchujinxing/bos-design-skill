...
**Modes** — Default to interactive H5 HTML/CSS/JS. Generate Taro/React-style guidance only when the user explicitly asks for developer handoff or real component integration.
...
**H5 interactions** — Include JavaScript for key component interactions (pickers, overlays, selection states, toast/dialog feedback, form validation, loading transitions, empty/error recovery). For full-screen demos, read `references/component-styles/basic/system.md` and render the Status Bar and Home Indicator. Keep interactions prototype-focused.
...
- Use iconfont as defined in `basic/icon.md` — do not hand-draw icons, arrows, checks, or status marks.
