...
| 形状 | 圆形 |
| 颜色 | `anti` |
| 阴影 | `0 2px 8px rgba(17,17,17,0.04)` — 一级阴影 |
...
自定义颜色仅影响开启状态的轨道背景色，关闭状态始终使用 `gray5` `gray5`。禁用状态下自定义颜色不生效，统一使用 `Branding-2` `Branding-2`（开启禁用）或 `gray8` `gray8`（关闭禁用）。
...
| Token | Value | Use |
|---|---|---|
| 一级阴影（灰色背景） | `0 2px 8px rgba(17,17,17,0.1), 0 0 1px rgba(17,17,17,0.06)` | 滑块投影（双层阴影，主阴影 + 微边缘） |
...
- 不要省略滑块的投影效果 `0 2px 8px rgba(17,17,17,0.04)`。
